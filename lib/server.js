...
const BROWSER_FORCE_RESTART_PERIOD = process.env.BROWSER_FORCE_RESTART_PERIOD || 1800000;
...
server.disconnectBrowserIfBrowserShouldBeRestarted = function () {
	...
	let time = new Date().getTime() - this.lastRestart;
	util.log(`disconnectBrowserIfBrowserShouldBeRestarted --> time passed since last restart ${time}`);

	if (new Date().getTime() - this.lastRestart > BROWSER_FORCE_RESTART_PERIOD) {
		util.log(`1 hour restart, current browser requests in flight ${this.browserRequestsInFlight}`);
	}
...
server.finish = function (req, res) {
	...
	util.log(`finish --> time passed since last restart ${time}`);

	if (new Date().getTime() - this.lastRestart > BROWSER_TRY_RESTART_PERIOD) {
		util.log(`restart should kick in, with requsts in flight ${this.browserRequestsInFlight}`);
	}
